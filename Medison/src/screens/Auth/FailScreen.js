...
function FailScreen() {
  ...
  const handleRescan = () => {
    // '다시 스캔하기' 버튼 클릭 시 FingerPrintScreen으로 돌아가기
    navigation.navigate('FingerPrint');
  };

  const handleNewRegistration = () => {
    // '신규 등록하기' 버튼 클릭 시 회원가입 또는 관련 화면으로 이동
    Alert.alert("신규 등록", "신규 등록 기능은 아직 구현되지 않았습니다.");
    // navigation.navigate('Register'); // 회원가입 화면이 있다면
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.description}>인증에 실패하였습니다.</Text>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity style={styles.registerButton} onPress={handleNewRegistration}>
            <Text style={styles.buttonText}>신규 등록하기</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
